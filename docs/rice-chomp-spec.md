...
- **Zero third-party runtime requests.** No CDN, no Google Fonts, no remote scripts, no remote images, no analytics — nothing the page fetches may leave the VPS. Sound is generated at build time by the repo's existing `scripts/gen-sfx.mjs` pipeline and played through `src/lib/sound.ts`.
  - **EVERY NEW SITE-WIDE PROVIDER MUST BE CHECKED AGAINST `src/lib/playSurfaces.ts`.** *Added 2026-08-04, Phase 5; the route in it is `/games/chomp` since Phase 7 — see* Route and information architecture. Four providers mounted in `app/layout.tsx` are now scoped off the game route through that one list — the chopstick cursor, the Konami rice dump, the rice-particle field and page translation — and the fourth is the instructive one. The first three were scoped off because they fight a maze game: they hide the cursor, listen on `window` for keys that are now primary game controls, and drape grains over the play area. **Translation was scoped off because its script was the only thing standing between this route and the acceptance criterion above.** That is the general lesson: anything added to `layout.tsx` is added to the games too, the cost is invisible in `layout.tsx` itself, and nobody finds it by reading the file — it is found by building the page and measuring it. Ask the question when the provider is added, not a phase later.
...
*Added 2026-08-05, Phase 7. The game moved, and this section exists because the move
is the kind that breaks things silently.*

**RICE CHOMP lives at `/games/chomp`.** It was at `/chomp` from Phase 2 until Phase 7,
when the site's three games — which had been at `/`, `/chomp` and `/play`, i.e. nowhere
near each other — moved under one `/games` parent with an index page above them. The
canonical scheme is `/games/<slug>`, with no exceptions: `chomp`, `grains`, `catch`.

- **`/chomp` still 308s to `/games/chomp`**, and will keep doing so. The path had never
  been shared, so it could in principle have moved bare — but it RETURNED 200, and "no
  path that resolved yesterday 404s today" is worth more as a rule with no exceptions
  than as a rule with one. `/play` (public since it shipped) and `/home` redirect the
  same way. `/` does not: it still returns 200 and now serves the home page instead of
  the Grains Game, which is a change to what a shared link *shows* and not a broken
  link.
- **`/games/chomp?level=N`** is the debug affordance, unchanged in every respect but
  the prefix.

**MOVING THIS ROUTE IS A TWO-FILE CHANGE, AND THE SECOND FILE FAILS SILENTLY.**
`src/lib/playSurfaces.ts` matches route paths EXACTLY. A rename that does not update it
throws nothing, reddens no test that is not looking for it, and renders a page that
appears completely healthy — while the Google Translate script comes back (re-breaking
the zero-third-party-request criterion in *Acceptance criteria*) and the chopstick
cursor, the Konami listener and the rice-particle field drape themselves back over a
live board. There is nothing to notice. `test/play-surfaces.test.ts` exists so that the
next move cannot be silent: it pins each game's play-surface status by name and asserts
that every entry in the list is a route that exists on disk.

**"IS A GAME" AND "IS A PLAY SURFACE" ARE DIFFERENT QUESTIONS.** There are three games
and exactly one is on the list. The obvious Phase 7 instruction — add every game route
to `playSurfaces.ts` — was wrong, and adopting it would have broken two shipped games:

- `/games/catch` (Catch A Grain) — you catch the grains **with** the chopstick cursor.
  Scoping the ambient decoration off it deletes the game's controller.
- `/games/grains` (the Grains Game) — same cursor, plus `globals.css`'s
  `.grains-play-area` styles a custom one for it, and the particles are part of the page.
- `/games/chomp` — on the list, because arrow keys are its controls, a chopstick over a
  maze HUD is wrong, and translation was the last third-party request standing between
  it and its acceptance criterion.
...
Suites: `test/chomp-{maze,movement,pests,cornering,levels,kiting,difficulty,audio,board,score,db}.test.ts`,
plus `test/canvas2d-shim.ts` and its own `canvas2d-shim.test.ts`, and — since Phase 7 —
`test/play-surfaces.test.ts`, which is not a game test at all but the guard on the route
this game is served from (see *Route and information architecture*).
...
**`chomp-db.test.ts` is the write path** (*added 2026-08-05, with the removal of the
country board*): the one suite that opens a database, and it opens a throwaway one in a
temp directory. It asserts that there are exactly two `chomp_*` tables — a third board
must not grow back unnoticed — that ranking is one row per player at their best score,
that a resubmitted run is a no-op reporting the truth, and that `country_code` reaches the
board as the flag column with a GeoIP miss still holding a rank. Phase 6 verified all of
that by hand against a running build (plan §10.3); this is the version that runs on every
commit.
...
**Amended 2026-08-05, hours after it shipped: THERE IS ONE BOARD, NOT TWO.** The top 50
players by best single run, with the player's country flag as a COLUMN beside their name.
The country board — top 100 countries, the second of the two boards this section used to
describe — is gone: query, table, component, tab strip and HUD button, deleted rather than
disabled. Lito's decision, and it also settles the "per-country top 100" ambiguity flagged
below by deleting the question rather than answering it. Everything that was true of the
*panel* stays true of it: same overlay/docked forms, same pause rule, same gutter sizing,
same validation. What follows is written as it now stands; where the removal changed a
reason rather than just a line, the old one is kept and marked.
...
- Views: **ONE board — the top 50 players — plus personal best in `localStorage`.** *Amended 2026-08-05. The original line read "global top 100, per-country top 100", and the second half of it was ambiguous: a board of the top 100 COUNTRIES, or the top 100 players within your own country? Both readings fit the sentence. It was built as the first, mirroring the grains game's two boards, and then removed — so the sentence no longer has to mean anything. Nothing on this game is ranked per country.*
  - **Ranked by each player's BEST RUN — one row per player, not one row per run.** A player who farms a memorised route a hundred times occupies one line with their best number on it, which is remedy 2 of the pattern-ability list below, already spent. It is also why a SUM was never the ranking anywhere on this game: a sum ranks whoever played most rather than whoever played best, and it is the one number a script can inflate without ever needing a good run. *This rule predates the removal and survived it deliberately.*
  - **50 rows, not 100, and it is only a `LIMIT`.** `getTopPlayers()` is one indexed read down `idx_chomp_players_best` with no filter and no post-processing, so the row count is the caller's number and nothing else changes with it. That was not true of the country board it replaced, where a GeoIP miss had to be filtered out before ranking and the query therefore over-fetched by one to keep the page full.
  - **The flag is a COLUMN, from the same nginx GeoIP headers, in the same treatment the grains board uses** — `flagEmoji` from `@/lib/grains/flag`, imported rather than copied, ISO-2 → regional indicators, globe for a code it does not recognise.
  - `isUnknownCountry` stays in `@/lib/grains/flag`, where this phase moved it from inside `CountryLeaderboard.tsx`. The grains board is its only caller again; it is not moved back, because a second edit to shipped grains code to undo a no-behaviour-change refactor is churn, and the module is the right home for it regardless.

### The panel, and the one rule that is not cosmetic

- **ONE button in the HUD bar** — "Board" — beside score, lives, level and pests, because that is where a player already looks for a number about their run. It toggles the panel. *Amended 2026-08-05: there were two, one per board, and the panel had a tab strip. With one board there is nothing to choose between, so the buttons became one button and the tab strip went with the country board rather than surviving as a one-tab row.*
...
- **A control whose caption toggles must not change size.** *Found by measuring, and it is the Phase 6 version of the §9.2 bugs.* "Resume" is 89px against "Pause"'s 84, and with two more buttons in it the HUD row sat within 11px of its wrap point at 1024×1366 — so PAUSING wrapped the row, took 44px out of the play row and resized the maze mid-run, and it did not reliably come back. Fixed twice over: the row gained real margin (`sm:gap-x-6`, 32px across four gaps) and the button gained a width floor. Both were needed; the floor alone leaves the row on the boundary and the margin alone leaves the caption jitter.
  - *Both fixes stay after the 2026-08-05 removal, which took one of those two board buttons out of the row.* The removal moves the row further from its wrap point, never nearer — but it is not what makes Pause safe, and a control whose caption toggles must not change size no matter how much room is going spare. The remaining board button needs no floor of its own: its caption is the same five characters open or closed.
...
- Zero third-party network requests, verified in the network tab. Self-hosted images under `public/chomp/` are within budget (500 KB total, 300 KB per file). Sound is within the same rule and is `public/sfx/chomp-*.wav`, 184 KB, synthesized in-repo.
  - **Passing as of 2026-08-04, Phase 5, and it was not before.** `layout.tsx` mounts `TranslateProvider` on every page, which loaded `https://translate.google.com/translate_a/element.js` — the only external host in `/chomp`'s HTML, present in the Phase 4 build too, and never asked for by anything in the game. It is now scoped off play surfaces through `src/lib/playSurfaces.ts`, the same list the chopstick cursor and the other decorations use. Measured on the built page:
    ...
    - **Re-measured for Phase 6**, because the leaderboard is the first thing on this page that makes a network request at all: 49–50 requests per load with both boards opened, **every one same-origin, zero third-party**. The leaderboard is `/api/chomp/leaderboard` on this host and nothing else — which is also why the plan chose HTTP on this vhost over anything that would have needed a new nginx block.
      - **Re-measured on the ONE-BOARD build** (`b5d6dd2`, 2026-08-05, headless Chrome over CDP against the preview on :3099, cache disabled): **48–49 requests per load, 49–50 with the board open, every one same-origin, zero third-party.** Desktop 1920×1080 (docked form) 49 → 50; phone 390×844 (overlay form) 48 → 49. **Opening the board costs exactly ONE request** at both — the single `/api/chomp/leaderboard` GET — which is also the standing proof that the in-flight share still holds: both forms of the panel are mounted and only one of them fetches. The same probe read 0 elements with `role="tab"` and one button captioned "Board", so the removal is confirmed in a real production build rather than only in the source.
      - **Re-measured for Phase 7, STATICALLY ONLY, and the gap is recorded rather than papered over.** *2026-08-05.* The route moved to `/games/chomp`, which is exactly the change that could have silently switched the translate script back on, so the criterion had to be re-checked. Measurement #1 was re-run against a production build on :3099 and passes: **0 external fetching elements, no `translate.google.com` script tag, no widget mount point** on `/games/chomp`. **Measurement #2 — the runtime CDP pass, which is the one that can tell a string in a chunk apart from a request — DID NOT RUN**: the only Chrome on the box cannot start (`libatk-1.0.so.0` missing) and installing it needs sudo. So the strongest available evidence this phase is static plus a control, not the network tab.
        - **The control is what makes the static pass worth anything.** A probe that reports "clean" everywhere is measuring nothing, so the same instrument was pointed at `/`, `/games` and `/games/grains`: all three still carry the translate element, and `/games/chomp` does not. The absence is therefore real rather than an instrument that cannot see. `test/play-surfaces.test.ts` covers the mechanism on every commit, which is the cheaper and more durable half.
        - **Anyone with a working headless Chrome should re-run measurement #2 on this route.** It is the only outstanding verification from this phase.
